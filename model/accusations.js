...
const components = require('../components/messages/winnerMessage')
const getAccessToken = require('./tokens').getAccessToken

let accuseAttemptCount = 0

async function getAccusationOptions(option) {
    let answerList = []
...
async function findAccused(request, response) {
    ...
    if (JSON.stringify(lastAccusation[0]) === JSON.stringify(currentKiller[0])) {
        response.send(components.generateWinnerMessage(currentAccuseCountUser + accuseAttemptCount))
    } else {
